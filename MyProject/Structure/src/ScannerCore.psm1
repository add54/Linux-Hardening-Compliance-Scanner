...
function Get-SystemInfo {
    ...
    try {
        ...
        if (Test-Path '/etc/os-release') {
            ...
            $info.Distribution = Get-Content '/etc/redhat-release'
            $info.ID = 'rhel'
        } elseif (Test-Path '/etc/debian_version') {
            $info.Distribution = "Debian $(Get-Content '/etc/debian_version')"
            $info.ID = 'debian'
        } else {
        ...
        $info.Architecture = (uname -m).Trim()
        
        # Uptime
        $uptime = (Get-Content '/proc/uptime' -ErrorAction SilentlyContinue) -split ' ' | Select-Object -First 1
        if ($uptime) {
            $info.Uptime = [TimeSpan]::FromSeconds([double]$uptime)
        }
        
        # CPU
        if (Test-Path '/proc/cpuinfo') {
            $cpuInfo = Get-Content '/proc/cpuinfo'
            $info.CPUCount = ($cpuInfo | Select-String '^processor' | Measure-Object).Count
            $info.CPUModel = ($cpuInfo | Select-String '^model name' | Select-Object -First 1).ToString().Split(':')[1].Trim()
        }
        
        # Memory
        if (Test-Path '/proc/meminfo') {
            $memInfo = Get-Content '/proc/meminfo'
            $totalMem = ($memInfo | Select-String '^MemTotal:' | ForEach-Object { [regex]::Matches($_, '\d+').Value[0] })[0]
            $info.TotalMemoryMB = [math]::Round([int]$totalMem / 1024)
        }
        
        # Disk
        $diskInfo = df -h / | Select-Object -Skip 1
        if ($diskInfo) {
            $info.DiskUsage = ($diskInfo -split '\s+')[4]
            $info.DiskAvailable = ($diskInfo -split '\s+')[3]
        }
        
        # Network
        $info.IPAddresses = (Get-NetIPAddress -AddressFamily IPv4 | Where-Object { $_.PrefixOrigin -ne 'WellKnown' }).IPAddress
        $info.DefaultGateway = (Get-NetRoute -DestinationPrefix '0.0.0.0/0' -ErrorAction SilentlyContinue).NextHop
        
        # Users
        $info.UserCount = (Get-Content '/etc/passwd' | Measure-Object).Count
        $info.LoggedInUsers = (who | Measure-Object).Count
        
        # Last boot
        $info.LastBoot = (Get-Date).AddSeconds(-[double]$uptime)
        
        # Security modules
        $info.SELinux = (Get-Command 'sestatus' -ErrorAction SilentlyContinue) -ne $null
        $info.AppArmor = (Get-Command 'aa-status' -ErrorAction SilentlyContinue) -ne $null
        
        # Package manager
        $info.PackageManager = if (Get-Command 'apt' -ErrorAction SilentlyContinue) {
            'apt'
        ...
        # Init system
        $info.InitSystem = if (Test-Path '/run/systemd/system') {
            'systemd'
        } elseif (Test-Path '/sbin/init') {
            'sysvinit'
        } else {
            'unknown'
        }
        ...
        # Container detection
        $info.IsContainer = Test-Path '/.dockerenv' -or 
                           (Test-Path '/proc/1/cgroup' -and 
                            (Select-String -Path '/proc/1/cgroup' -Pattern 'docker|lxc').Success)
        
        # Cloud detection
        $info.IsCloud = $false
        $info.CloudProvider = 'None'
        
        # Try to detect cloud providers
        $cloudTests = @(
            @{ Provider = 'AWS'; Test = { Test-Path '/sys/hypervisor/uuid' -and (Get-Content '/sys/hypervisor/uuid' -ErrorAction SilentlyContinue).StartsWith('ec2') } },
            @{ Provider = 'Azure'; Test = { Test-Path '/sys/class/dmi/id/chassis_asset_tag' -and (Get-Content '/sys/class/dmi/id/chassis_asset_tag' -ErrorAction SilentlyContinue) -eq '7783-7084-3265-9085-8269-3286-77' } },
            @{ Provider = 'GCP'; Test = { Test-Path '/sys/class/dmi/id/product_name' -and (Get-Content '/sys/class/dmi/id/product_name' -ErrorAction SilentlyContinue) -eq 'Google Compute Engine' } }
        )
        
        foreach ($test in $cloudTests) {
            if (& $test.Test) {
                $info.IsCloud = $true
                $info.CloudProvider = $test.Provider
                break
            }
        }
