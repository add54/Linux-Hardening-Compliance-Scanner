...
# Check PowerShell version and set platform variables if not available
...
$Global:ScannerConfig = @{
    ...
    FixMode = $Fix
    Verbose = $VerbosePreference -ne 'SilentlyContinue'  # Use built-in Verbose
    Quiet = $Quiet
    NoColor = $NoColor
    Export = $Export
    Exclude = $Exclude
...
function Initialize-Scanner {
    ...
    if ($IsLinux -eq $false -and $IsMacOS -eq $false) {
        Write-Warning "This scanner is designed for Linux systems. Running on Windows may not work correctly."
        Write-Warning "Some checks will be skipped or simulated."
    }

    # Check for required commands (only on Linux)
    if ($IsLinux) {
    ...
    if ($Global:ScanResults.SystemInfo.Distribution) {
        Write-Host "Distribution: $($Global:ScanResults.SystemInfo.Distribution)" -ForegroundColor Gray
    }
    if ($Global:ScanResults.SystemInfo.KernelVersion) {
...
function Invoke-SecurityScan {
    ...
    try {
        ...
        if ($modulesToRun.Count -gt 0) {
            ...
            Write-Host "No modules to run (check profile configuration)`n" -ForegroundColor Yellow
        }
...
# Output formatter functions (simplified versions)
function Format-TextReport {
    ...
    foreach ($check in $Results.Checks) {
        ...
        Write-Host "[$($check.ID)] $($check.Name)" -ForegroundColor $color
        Write-Host "  Status: $($check.Status)" -ForegroundColor $color
...
"@
    ...
    if ($Global:ScannerConfig.OutputFile) {
        $html | Out-File $Global:ScannerConfig.OutputFile
        Write-Host "HTML report saved to: $($Global:ScannerConfig.OutputFile)" -ForegroundColor Green
    } else {
        $html
    }
}
...
function Format-CsvReport {
    ...
    $csv = "ID,Name,Category,Severity,Status,Message`n"
    foreach ($check in $Results.Checks) {
        $csv += """$($check.ID)"",""$($check.Name)"",""$($check.Category)"",""$($check.Severity)"",""$($check.Status)"",""$($check.Message.Replace('""', '""""'))""`n"
    }

    if ($Global:ScannerConfig.OutputFile) {
        $csv | Out-File $Global:ScannerConfig.OutputFile
        Write-Host "CSV report saved to: $($Global:ScannerConfig.OutputFile)" -ForegroundColor Green
    } else {
        $csv
    }
}
...
<SecurityScan>
    <Report>
        <ScanID>$($Results.ScanID)</ScanID>
        <Timestamp>$(Get-Date -Format 'yyyy-MM-ddTHH:mm:ss')</Timestamp>
        <Profile>$($Global:ScannerConfig.Profile)</Profile>
        <Summary>
            <TotalChecks>$($Results.Summary.TotalChecks)</TotalChecks>
            <Passed>$($Results.Summary.Passed)</Passed>
            <Failed>$($Results.Summary.Failed)</Failed>
            <Warning>$($Results.Summary.Warning)</Warning>
            <ComplianceScore>$($Results.ComplianceScore)</ComplianceScore>
        </Summary>
        <Checks>
        </Checks>
    </Report>
...
"@
    ...
    $checksNode = $xml.SelectSingleNode('//Checks')
    foreach ($check in $Results.Checks) {
        $checkNode = $xml.CreateElement('Check')
        $checkNode.SetAttribute('ID', $check.ID)
        $checkNode.SetAttribute('Name', $check.Name)
        $checkNode.SetAttribute('Category', $check.Category)
        $checkNode.SetAttribute('Severity', $check.Severity)
        $checkNode.SetAttribute('Status', $check.Status)

        $messageNode = $xml.CreateElement('Message')
        $messageNode.InnerText = $check.Message
        $checkNode.AppendChild($messageNode)

        if ($check.Remediation) {
            $remediationNode = $xml.CreateElement('Remediation')
            $remediationNode.InnerText = $check.Remediation
            $checkNode.AppendChild($remediationNode)
        }
    ...
    if ($Global:ScannerConfig.OutputFile) {
        $xml.Save($Global:ScannerConfig.OutputFile)
        Write-Host "XML report saved to: $($Global:ScannerConfig.OutputFile)" -ForegroundColor Green
    } else {
        $xml.OuterXml
    }
}
...
function Calculate-ComplianceScore {
    ...
    return [math]::Round(($passed / $Results.Count) * 100, 2)
}
